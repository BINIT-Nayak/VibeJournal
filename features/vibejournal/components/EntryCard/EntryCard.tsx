  ...
  return (
    <article className={styles.entryCard} style={{ "--mood-color": moodMeta.color } as CSSProperties}>
      ...
      <div className={styles.entryMetrics}>
        ...
        {typeof entry.exerciseMinutes === "number" && <span>Movement {entry.exerciseMinutes}m</span>}
      ...
      {entry.photoNames && entry.photoNames.length > 0 && (
        <div className={styles.attachmentRow}>
          {entry.photoNames.map((name) => <span key={name}>{name}</span>)}
        </div>
      )}
      ...
      {(onView || onEdit || onArchive || onDelete) && (
        <div className={styles.entryActions}>
          {onView && <button onClick={() => onView(entry.id)} type="button">View</button>}
          {onEdit && <button onClick={() => onEdit(entry)} type="button">Edit</button>}
          {onArchive && <button onClick={() => onArchive(entry)} type="button">{entry.archivedAt ? "Restore" : "Archive"}</button>}
          {onDelete && <button onClick={() => onDelete(entry.id)} type="button">Delete</button>}
        </div>
      )}
    </article>
  );
